...
char * music[6] = {
            ...
            (char *) music3_mod_bin,
            "Song: Megaman Music!",
            "Song: Slash Your Friends",
            "Song: Technostyle III"};
...
void LoadManagerCfg()
{
    ...
    {
        manager_cfg.opt_flags |= OPTFLAGS_PLAYMUSIC; // enabled by default
	manager_cfg.gui_mode = 4;
	manager_cfg.noBDVD = 2;
	manager_cfg.bg_gear = 1;
	manager_cfg.priority = 1;
    }
...
s32 main(s32 argc, const char* argv[])
{
    ...
    if(use_cobra) {
        struct stat s;
        sprintf(temp_buffer, PLUGIN_ISO, self_path);
...
*/
    ...
    if(manager_cfg.priority) {
        DrawButton1_UTF8((848 - 520) / 2, y2, 520, language[DRAWTOOLS_PRIORITYON], (flash && select_option == 8));}
    else {
        DrawButton1_UTF8((848 - 520) / 2, y2, 520, language[DRAWTOOLS_PRIORITYOFF], (flash && select_option == 8));}
    ...
    */
    ...
    if((old_pad & (BUTTON_L1 | BUTTON_R1 | BUTTON_SELECT | BUTTON_START)) == (BUTTON_L1 | BUTTON_R1 | BUTTON_SELECT | BUTTON_START)) {
        struct stat st;
        if (stat("/dev_usb000", &st) == 0) {
	    DrawDialogTimer("Copying explore_plugin.sprx and libfs.sprx!\n\n                      Please wait!", 1000);
	    file_copy("/dev_flash/vsh/module/explore_plugin.sprx", "/dev_usb000/explore_plugin.sprx");
	    file_copy("/dev_flash/sys/external/libfs.sprx", "/dev_usb000/libfs.sprx");
	    DrawDialogOK("Copied successfully!");
	}
	else {
            DrawDialogOK("USB flash drive not found!");
	}
	return;
    }
            ...
            case 4:
                ...
                if(!manager_cfg.noBDVD)
                      sprintf(temp_buffer, "%s\n\n%s\n\n%s?", language[DRAWTOOLS_WITHBDVD],
                            "Exiting to the XMB: launch Gamesonic Manager again",
                            "Saliendo al XMB: lanza Gamesonic Manager de nuevo");
                else if(manager_cfg.noBDVD == 2)
    ...
    */
    ...
    if(gui_mode == 0 || gui_mode == 2  || gui_mode == 5) {
	    ...
	    case 2:
            ...
            case 4:
                DrawButton2_UTF8(x2, y2, 0, "4 x 5", 1);
            break;
	    ...
	    case 5:
            ...
            case 8:
                DrawButton2_UTF8(x2, y2, 0, "6 x 3", 1);
            break;
	    ...
	    case 9:
            ...
            case 11:
                DrawButton2_UTF8(x2, y2, 0, "7 x 4", 1);
            break;
			
            case 12:
                DrawButton2_UTF8(x2, y2, 0, "8 x 4", 1);
            break;
    ...
    else if(gui_mode == 0 || gui_mode == 2) {
	x2 = DrawButton1_UTF8(x + 32, y2, 320, "Background Gear", (flash && select_option == 5)) + 16;
	if((manager_cfg.bg_gear)) DrawButton2_UTF8(x2, y2, 0, "Yes", 1); else DrawButton2_UTF8(x2, y2, 0, "No", 1);
    ...
    int n;
    for(n = 0; n < ((gui_mode == 0 || gui_mode == 2 || gui_mode == 5) ? 2 : 3) ; n++) {
    ...
    if(new_pad & (BUTTON_CROSS | BUTTON_CIRCLE)) {
        ...
        switch(select_option) {
            ...
            case 3:
                if(gui_mode == 0 || gui_mode == 2 || gui_mode == 5) {
                    grid_mode++; if(grid_mode > 12) grid_mode = 0;  //+++++++++++++++++++++++++++++++++
                    grid_mode = configure_grid((u8) grid_mode);
                    manager_cfg.grid_mode = grid_mode;
    ...
    if(new_pad & BUTTON_UP) {
        ...
        if(select_option < 0) {
            ...
            if(gui_mode == 0 || gui_mode == 2 || gui_mode == 5) select_option = 6; else select_option = 5;
    ...
    if(new_pad & BUTTON_DOWN) {
        ...
        if(select_option > ((gui_mode == 0 || gui_mode == 2 || gui_mode == 5) ? 6 : 5)) {
