...
t_lngstr lang_strings[] = 
{
   ...
   // { DRAWTOOLS_SECENABLE, "DRAWTOOLS_SECENABLE"     , "Press To Enable Syscall Security" },
    ...
    { DRAWTOOLS_COPYFROM, "DRAWTOOLS_COPYFROM"     , "Copy from /dev_usb/play to Gamesonic Manager folder"},
    ...
    { DRAWTOOLS_PRIORITYON, "DRAWTOOLS_PRIORITYON", "Priority: Enabled" },
    { DRAWTOOLS_PRIORITYOFF, "DRAWTOOLS_PRIORITYOFF", "Priority: Disabled" },
    ...
    { PKG_COPYING, "PKG_COPYING", "Copying .PKG file to Gamesonic Manager folder..."},
...
int open_language (int lang, char * filename)
{
    ...
    if(lang >= 10) { // custom language (external filename)
        if(!stat(filename, &s))
