...
#define SYSCALL_BASE                    0x8000000000363BE0ULL //done
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR           0x80000000002A1480ULL  // peek(SYSCALL_BASE + NEW_POKE_SYSCALL*8) -> peek(0x363B60 + 813*8)

#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x29FB98 + 0x8) // SYSCALL (838) + 8 -> peek(SYSCALL_BASE + 838*8) -> peek(0x363B60 + 0x1A30)

#define LV2MOUNTADDR_475 0x8000000000479FC0ULL // Old: 0x8000000000479FE0ULL - FIXED
...
int is_firm_475(void)
{
   ...
   if(toc == 0x800000000034FBB0ULL && peekq(0x80000000002FCB68ULL) == 0x323031352F30342FULL)
   {
      return 1;
   }
   else
...
int is_firm_476(void)
{
   ...
   u64 toc;
   toc = peekq(0x8000000000003000ULL);
   if(toc == 0x800000000034FBB0ULL && peekq(0x80000000002FCB68ULL) == 0x323031352F30382FULL)
   {
      return 1;
   }
   else
...
int is_firm_478(void)
{
   ...
   u64 toc;
   toc = peekq(0x8000000000003000ULL);
   if(toc == 0x800000000034FBB0ULL && peekq(0x80000000002FCB68ULL) == 0x323031352F31322FULL)
   {
      return 1;
   }
   else
...
int is_firm_481(void)
{
   ...
   u64 toc;
   toc = peekq(0x8000000000003000ULL);
   if(toc == 0x800000000034FBB0ULL && peekq(0x80000000002FCB68ULL) == 0x323031362F31302FULL)
   {
      return 1;
   }
   else
...
int is_firm_483(void)
{
   ...
   u64 toc;
   toc = peekq(0x8000000000003000ULL);
   if(toc == 0x800000000034FBB0ULL && peekq(0x80000000002FCB68ULL) == 0x323031382F30392FULL)
   {
      return 1;
   }
   else
...
{    
    ...
    _poke32(0x56600, 0x60000000);             // Original: 0x419D00C02F840004ULL -> 0x419E00D860000000ULL
    PATCH_JUMP(0x56608, 0x566A0);             // Original: 0x419E00ACE8610188ULL -> 0x2F84000448000098ULL
    ...
    */
    ...
    _poke32(0x297638, 0xF821FF61); // free openhook Rogero 4.30 (put "stdu    %sp, -0xA0(%sp)" instead   "b       sub_2E9F98")
