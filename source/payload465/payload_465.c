...
int is_firm_465(void)
{
   ...
   if(toc == 0x800000000034F960ULL && peekq(0x80000000002FC938ULL) != 0x323031342F31312FULL)
   {
      return 1;
   }
   else
...
//Remove Lv2 memory protection
    ...
    /*if(is_rebug)
	{
	  lv1poke(0x377828     , 0x0000000000000001ULL);
      lv1poke(0x377828 + 8 , 0xE0D251B556C59F05ULL);
      lv1poke(0x377828 + 16, 0xC232FCAD552C80D7ULL);
      lv1poke(0x377828 + 24, 0x65140CD200000000ULL);
    }*/
    ...
    _poke32(0x56600, 0x60000000); // done
    PATCH_JUMP(0x56608, 0x566A0); // done
	...
	pokeq(0x80000000002302F0ULL, 0x38600000F8690000ULL ); // fix 0x8001002B / 80010017 errors
...
/*#define OFFSET_FIX	 0x05658C // is the same function of offsets of the Master Deank
#define OFFSET_2_FIX	 0x05A65C
#define OFFSET_FIX_BS	 0x164294
#define OFFSET_FIX_17	 0x058DB0
#define OFFSET_FIX_2B	 0x055C5C
#define OFFSET_FIX_3C	 0x26FDE0

void fix_error_465()
{

_poke	(OFFSET_FIX_2B	 , 0x386000004E800020); 
_poke	(OFFSET_FIX	 , 0x63FF003D60000000); // error 3D
_poke32	(OFFSET_FIX + 0x74	 , 0x60000000	 ); 
_poke32	(OFFSET_FIX + 0x74 + 0x8 , 0x48000098	 );
_poke	(OFFSET_FIX + 0xC8 , 0x3BE000004BFFFF0C); // error 3E 
_poke32	(OFFSET_FIX_17	 , 0x60000000	 ); 
_poke32	(OFFSET_2_FIX	 , 0x60000000	 ); // error 09
_poke	(OFFSET_FIX_3C	 , 0x386000007C6307B4); 
_poke32	(OFFSET_FIX_3C + 0x8	 , 0x4E800020	 );

_poke32(OFFSET_FIX_BS	 , 0x38600000);
_poke32(OFFSET_FIX_BS + 0xBB4	, 0x4865E6E9);
_poke32(OFFSET_FIX_BS + 0x3DD0	, 0x4865B5A1);
